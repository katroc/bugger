...
A comprehensive Model Context Protocol (MCP) server that transforms how you manage project tracking. Seamlessly create, track, and manage bugs and improvements (including lightweight TODOs) using natural language conversations with your AI assistant.
...
### Core Management
- **create_todo**: Quickly create a lightweight TODO (stored as an improvement) with minimal fields. Optionally provide `linkTo` to relate it to an existing item.
- **create_item**: Create new bugs or improvements with detailed information including title, description, and priority.
- **list_items**: List bugs or improvements with optional filtering by status, priority, component, or category.
- **update_item_status**: Update the status of individual items (bugs or improvements). Also accepts TODO statuses for improvements.
- **bulk_update_items**: Update multiple items in a single operation.

### Search & Analytics
- **search_items**: Advanced search across bugs and improvements with filtering, sorting, and pagination.
- **get_statistics**: Generate project statistics for bugs, improvements, or all items.
- **get_related_items**: Find items related to a specific bug or improvement.
...
### Item Management
- Quick TODO: use the `create_todo` tool with `{ "title": "Write small task", "status": "Todo" }`. Optionally add `linkTo` to relate to an existing bug or improvement.
- List TODOs: use `list_items` with `{ "type": "improvement", "view": "todos" }`. Other views: `blocked`, `done`, `today`.
...
- "Give me project statistics to see how many bugs and improvements we have"
- "Find all high-priority items that are currently open across bugs and improvements"
...
- **Code Snippets**: Relevant code sections that relate to bugs or improvements
...
- **ImprovementManager**: Tracks technical debt, code quality improvements, and optimization tasks (including TODOs)
